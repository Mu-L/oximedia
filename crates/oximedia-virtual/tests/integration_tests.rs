...
fn test_led_wall_workflow_integration() {
    ...
    let mut workflow = LedWallWorkflow::with_panel_resolution((TEST_W, TEST_H)).unwrap();
...
fn test_hybrid_workflow_integration() {
    ...
    let mut workflow = HybridWorkflow::with_resolution(TEST_W, TEST_H).unwrap();
...
fn test_ar_workflow_integration() {
    let mut workflow = ArWorkflow::with_resolution(TEST_W, TEST_H).unwrap();

    workflow.start("ar-test".to_string(), 0).unwrap();
...
fn test_full_pipeline_integration() {
    ...
    for i in 0..2 {
        ...
        // Track camera
        let camera_pose = vp.camera_tracker_mut().update(timestamp_ns).unwrap();
...
fn test_multi_camera_integration() {
    ...
    let vp = VirtualProduction::new(config).unwrap();

    assert!(vp.multicam_manager().is_some());

    let multicam = vp.multicam_manager().unwrap();
...
fn test_genlock_sync_integration() {
    ...
    let mut genlock = GenlockSync::new(config).unwrap();
